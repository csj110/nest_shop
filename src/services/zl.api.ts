...
export const zlApi = {
  ...
  fetchPDetail(p: SkuId) {
    return zlPost('womai.itemdetail.get', p).then(res => res.itemdetail[0]).then(({ goodsid, goodsname, prodescription, category }) => {
      const r = /(?<=src=[\'\"])(\S*)(?=[\'\"])/g
  ...
  fetchPInventory(p: FetchPInventoryParam): Promise<number> {
    return zlPost('womai.inventory.get', p).then(res => parseInt(res.Inventory[0].inventory));
  },
